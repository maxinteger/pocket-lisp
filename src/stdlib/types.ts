 ...
 */
export interface SChainRec<a> /* extends Cain */ {
  new (): Chain<a>

  // SChainRec m => ((a -> c, b -> c, a) -> m c, a) -> m b
  [chainRec]<b>(f: (a: a) => Chain<b>): Chain<b>
}
 ...
 */
export interface Monad<a> extends Chain<a> /*, Applicative */ {}
...
export interface Profunctor<b, c> extends Functor<b> {
  ...
  [promap]<a, d>(f: (a: a) => b, g: (c: c) => d): Bifunctor<a, d>
}
